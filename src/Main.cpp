...
#define DWMWCP_ROUND 2
    ...
    private:
        ...
        void RenderTopbar(ImVec2 size, float top_h)
        {
            ...
            TopNavButton("Dashboard", View::Dashboard, IconKind::Dashboard, 120.0f);
            ImGui::SameLine();
            TopNavButton("Live Betting", View::Live, IconKind::Live, 132.0f);
            ImGui::SameLine();
            TopNavButton("AI Picks", View::Picks, IconKind::Brain, 112.0f);
            if (!compact_topbar)
            {
                ImGui::SameLine();
                TopNavButton("Analytics", View::Analytics, IconKind::Chart, 118.0f);
                ImGui::SameLine();
                TopNavButton("Arbitrage", View::Arbitrage, IconKind::Arbitrage, 120.0f);
                ImGui::SameLine();
                TopNavButton("Watchlist", View::Watchlist, IconKind::Bell, 120.0f);
                ImGui::SameLine();
                TopNavButton("Settings", View::Settings, IconKind::Settings, 108.0f);
            }
        ...
        void RenderSidebar()
        {
            ...
            SidebarButton("Arbitrage", View::Arbitrage, "", IconKind::Arbitrage);
        ...
        void RenderDashboard(float width)
        {
            ...
            ImGui::BeginChild("live_card", ImVec2(0, 246), true);
            ...
            for (int i = 0; i < count; ++i)
            {
                if (i > 0)
                    ImGui::SameLine();
                RenderEventCard(*featured[static_cast<size_t>(i)], ImVec2(card_w, 174));
            }
            if (count == 0)
            ...
            ImGui::Spacing();
            ImGui::BeginChild("picks_card", ImVec2(0, 248), true);
        ...
        void RenderGameDetail(float width)
        {
            ...
            TextMuted((game->feed_age_label.empty() ? "Freshness unknown" : game->feed_age_label).c_str());
            ...
            else if (ImGui::BeginTable("detail_market_table", 6, ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_SizingStretchProp))
            {
                ...
                ImGui::TableSetupColumn("Movement", ImGuiTableColumnFlags_WidthStretch, 0.95f);
                ImGui::TableHeadersRow();
                for (const aegis::BetLink& link : game->bet_links)
                {
                    ImGui::TableNextRow();
        ...
        std::string SetupReadyCount() const
        {
            ...
            ready += !IsBoardStale() ? 1 : 0;
            return std::to_string(ready) + "/6";
        }
        ...
        std::string AdapterDetail(const std::string& key, const std::string& url, const std::string& empty_detail) const
        {
            ...
            const auto it = adapter_probes_.find(key);
            if (it != adapter_probes_.end() && it->second.checked)
                return it->second.detail.empty() ? trimmed : it->second.detail;
        ...
        std::vector<aegis::InfoItem> DataAdapterRows() const
        {
            ...
            return {
                {"Injury feed", "", AdapterStatus("injury", injury), "", AdapterDetail("injury", injury, "Add a verified injury provider URL to reduce availability uncertainty.")},
                {"Lineup feed", "", AdapterStatus("lineup", lineup), "", AdapterDetail("lineup", lineup, "Starting lineups and scratches remain manual checks until configured.")},
                {"News feed", "", AdapterStatus("news", news), "", AdapterDetail("news", news, "Breaking team news is not yet connected to confidence scoring.")},
                {"Props feed", "", AdapterStatus("props", props), "", AdapterDetail("props", props, "Player prop rows stay hidden until a verified provider is configured.")},
        ...
        void ExportWorkspaceReport()
        {
            ...
            file << "summary\tgames\t" << state_.games.size() << "\tbook lines " << CountAvailableBookLines() << "\n";
            ...
            for (const aegis::Prediction& prediction : state_.predictions)
            {
                file << "prediction\t" << TsvField(prediction.matchup) << '\t'
                     ...
                     << TsvField(prediction.pick + " / " + prediction.market + " / " + prediction.edge) << "\n";
            }
            for (const aegis::InfoItem& alert : state_.alerts)
            {
            ...
            int market_rows = 0;
            for (const auto& pair : MarketSnapshotPairs(1000))
            {
                if (market_rows++ >= 50)
                    break;
        ...
        void ExportPdfReport()
        {
            ...
            lines.push_back("Games: " + std::to_string(static_cast<int>(state_.games.size())) + " / Predictions: " + std::to_string(static_cast<int>(state_.predictions.size())));
        ...
        AdapterProbe ProbeAdapterUrl(const std::string& name, const std::string& url) const
        {
            AdapterProbe probe;
            probe.checked = true;
            const std::string trimmed = aegis::Trim(url);
            if (trimmed.empty())
            {
                ...
                probe.detail = name + " URL is empty.";
                return probe;
            }
            if (!IsHttpUrl(trimmed))
            {
                probe.status = "Invalid URL";
                probe.detail = name + " must start with http:// or https://.";
                return probe;
            }
            ...
            if (response.status_code >= 200 && response.status_code < 300)
            {
                ...
                probe.detail = "HTTP " + std::to_string(response.status_code) + " from " + trimmed + " (" + std::to_string(response.body.size()) + " bytes).";
                return probe;
            }

            probe.status = "HTTP " + std::to_string(response.status_code);
            probe.detail = name + " responded but was not a success status: " + trimmed;
            return probe;
        }
        ...
        void ValidateDataAdapters()
        {
            ...
            {
                AdapterProbe probe = ProbeAdapterUrl(target.name, target.url);
            ...
            status_ = "Data adapter validation finished: " + std::to_string(reachable) + "/" + std::to_string(configured) + " reachable.";
            aegis::AppendDiagnosticLine("data adapter validation reachable=" + std::to_string(reachable) + " configured=" + std::to_string(configured));
            AppendProviderHealthSnapshot("validate_feeds");
        }
        ...
        {
            std::ofstream file(WatchlistFile(), std::ios::trunc);
            if (!file)
                return;
            for (const std::string& id : watchlist_ids_)
            {
        ...
        void RenderMarketLinkCard(const aegis::Game& game, const aegis::BetLink& link, ImVec2 size)
        {
            ...
            TextGreen(link.available ? "Available" : "Link only");
            ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 50.0f);
        ...
        {
            const ImVec2 box(0, 142.0f);
            ImDrawList* draw = ImGui::GetWindowDrawList();
            const ImVec2 pos = ImGui::GetCursorScreenPos();
            const float width = ImGui::GetContentRegionAvail().x;
            draw->AddRectFilled(pos, ImVec2(pos.x + width, pos.y + box.y), Col(0.025f, 0.045f, 0.038f, 0.78f), 10.0f);
            draw->AddRect(pos, ImVec2(pos.x + width, pos.y + box.y), Col(0.77f, 0.87f, 0.81f, 0.10f), 10.0f);
            DrawGeneratedLogoBadge(draw, ImVec2(pos.x + 16.0f, pos.y + 24.0f), ImVec2(38.0f, 38.0f), title, IconKind::Search, false, false);
            ImGui::SetCursorScreenPos(ImVec2(pos.x + 62.0f, pos.y + 27.0f));
            ImGui::TextColored(V4(1, 1, 1, 1), "%s", title);
            ImGui::SetCursorScreenPos(ImVec2(pos.x + 62.0f, pos.y + 51.0f));
            TextMuted(detail);
            ImGui::Dummy(box);
        }
        ...
        void RenderPredictionTable(int limit, bool winner_column)
        {
            ...
            if (ImGui::BeginTable("predictions", 8, ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_SizingStretchProp))
            {
                ...
                for (const int index : indexes)
                {
                    ...
                    ImGui::TableSetColumnIndex(1); ImGui::TextUnformatted(p.matchup.c_str());
                    ImGui::TableSetColumnIndex(2); ImGui::TextUnformatted(p.market.c_str());
                    ImGui::TableSetColumnIndex(3); ConfidenceBar(p.confidence_value, p.confidence);
        ...
        void AttemptLogin(bool manual)
        {
            ...
            if (!response.error.empty())
            {
                ...
                status_ = "Could not reach auth service at " + config_.auth_base_url + ".";
                return;
            }
            ...
            if (response.status_code < 200 || response.status_code >= 300 || !parsed.ok || !parsed.value["ok"].AsBool(false))
            {
                ...
                if (parsed.ok)
                    status_ = "Login failed (" + std::to_string(response.status_code) + "): " + parsed.value["message"].AsString("Login failed.");
                else
        ...
        void BeginSportsRefresh(bool initial, const std::string& headline, const std::string& detail)
        {
            ...
            refresh_future_ = std::async(std::launch::async, [config, request_id]() {
                ...
                result.diagnostic = "native sports synchronized games=" + std::to_string(result.state.games.size()) + " predictions=" + std::to_string(result.state.predictions.size());
